...
function gplus_get_options() {
	$gplus_array=unserialize(get_option('gplus_options'));
	return $gplus_array;
}

function gplus_update_options(){
	$arrpost=$_POST;
	$arrpost=serialize($arrpost);
...
function gplus_update($content) {
    ...
    // is it a page
    if ($options['display_page'] == null && is_page()) {
        return $content;
    }
   ...
   if ($options['shortcode'] != '') {
 	return str_replace('[gplusbutton]', $button, $content);
   }
...
add_filter('the_content', 'gplus_update', 8);
